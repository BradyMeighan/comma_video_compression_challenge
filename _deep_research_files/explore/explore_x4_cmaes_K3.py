#!/usr/bin/env python
"""X4: CMA-ES with K=3 mask flips per pair on top 100 (vs K=2 in O2)."""
...
def main():
    ...
    for i, pi in enumerate(rank[:100]):
        ...
        gt_p = p.clone()
        flips = cma_es_mask_for_pair(gen, m, p, gt_p, posenet, device, K=3, pop=12, gens=18)
        if flips:
            if pi in new_mask_patches:
    ...
    print(f"X4: sb_mask={sb_mask}B sb_rgb={sb_rgb}B sb_total={sb_mask+sb_rgb}B "
          f"score={full['score']:.4f} delta={full['score']-score_bl:+.4f} ({time.time()-t0:.0f}s)")

    import csv
    with open(OUTPUT_DIR / "x4_cmaes_K3_results.csv", 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(["spec", "n_added", "sb_total", "score", "delta"])
        w.writerow(["x4_cmaes_top100_K3", n_added, sb_mask+sb_rgb, full['score'],
                    full['score']-score_bl])
