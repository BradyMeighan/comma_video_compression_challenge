#!/usr/bin/env python
"""
Best-combo: only pose_f1 patches (the proven winner), tested across tier
allocations. From chain experiment: pose_f2, seg_f2, dim0 all lose marginal
cost-benefit. Only extending pose_f1 coverage compounds.

Configs to test:
  A. top400_K7 + tail200_K3   — chain step 1 + step 5 (target)
  B. top400_K7 + tail200_K4   — slightly more on tail
  C. top500_K6 + tail100_K3   — flatter
  D. top600_K6                 — uniform K=6
  E. top600_K5                 — uniform K=5 (current sidecar_explore baseline)
  F. top300_K8 + next200_K5 + tail100_K3  — finer tier
  G. top400_K8 + tail200_K3   — slightly more on top
"""
...
def main():
    ...
    csv_path = OUTPUT_DIR / "best_combo_results.csv"
    with open(csv_path, 'w', newline='') as f:
        csv.writer(f).writerow(["spec", "pairs", "K_total", "sidecar_bytes",
                                 "score", "pose_term", "delta", "elapsed"])

    def run(spec, tiers):
        """tiers: list of (start, end, K) — pairs ranked [start:end] get K patches each."""
        all_patches = {}
        t0 = time.time()
    ...
    print("\n=== Best-combo configurations ===", flush=True)
    run("A_top400_K7+tail200_K3", [(0, 400, 7), (400, 600, 3)])
    run("B_top400_K7+tail200_K4", [(0, 400, 7), (400, 600, 4)])
    run("C_top500_K6+tail100_K3", [(0, 500, 6), (500, 600, 3)])
    run("D_top600_K6_uniform",    [(0, 600, 6)])
    run("E_top600_K5_uniform",    [(0, 600, 5)])
    run("F_top300_K8+next200_K5+tail100_K3", [(0, 300, 8), (300, 500, 5), (500, 600, 3)])
    run("G_top400_K8+tail200_K3", [(0, 400, 8), (400, 600, 3)])
    run("H_top400_K7+tail200_K2", [(0, 400, 7), (400, 600, 2)])
    run("I_top500_K7+tail100_K3", [(0, 500, 7), (500, 600, 3)])
