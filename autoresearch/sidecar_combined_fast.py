#!/usr/bin/env python
"""
Fast combined: minimal test of mask + RGB stacking.
- Skip RGB reference (already known: -0.0268 at 15.1KB)
- Use K=1 mask top100 (~110s, proven -0.0109)
- Then full RGB tier (350_K7+250_K2) on patched frames
- Compare combined score vs RGB-only baseline
"""
...
def main():
    ...
    rgb_baseline_known = -0.0268
    print(f"Known RGB-only best: delta={rgb_baseline_known:+.4f} (score={base['score']+rgb_baseline_known:.4f})", flush=True)
    ...
    print("\n=== Step 1: Find mask patches K=1 top100 ===", flush=True)
    t0 = time.time()
    mask_patches = {}
    for i, pi in enumerate(rank[:100]):
        ...
        if (i + 1) % 25 == 0:
            print(f"  ... {i+1}/100 pairs done ({time.time()-t0:.1f}s)", flush=True)
    sb_mask = mask_sidecar_size(mask_patches)
    print(f"  Mask done: {len(mask_patches)} pairs accepted, sb={sb_mask}B ({time.time()-t0:.1f}s)", flush=True)

    print("\n=== Step 2: Apply masks + regenerate frames ===", flush=True)
    ...
    print("\n=== Step 3: Find RGB patches on PATCHED frames (350_K7+250_K2) ===", flush=True)
    t1 = time.time()
    ...
    print(f"  Combined: sb_mask={sb_mask}B sb_rgb={sb_rgb}B sb_total={sb_mask+sb_rgb}B", flush=True)
    print(f"            score={full['score']:.4f} pose={full['pose_term']:.4f}", flush=True)
    print(f"            delta_vs_baseline={full['score']-base['score']:+.4f}", flush=True)
    print(f"            delta_vs_rgb_only={full['score']-base['score']-rgb_baseline_known:+.4f}",
          flush=True)
    print(f"  Step 3 elapsed: {time.time()-t1:.1f}s", flush=True)

    csv_path = OUTPUT_DIR / "combined_fast_results.csv"
    with open(csv_path, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(["spec", "sb_mask", "sb_rgb", "sb_total", "score",
                     "pose_term", "delta_baseline", "delta_vs_rgb_only"])
        w.writerow(["mask_K1_top100+rgb_350K7_250K2", sb_mask, sb_rgb, sb_mask + sb_rgb,
                     full['score'], full['pose_term'],
                     full['score'] - base['score'],
                     full['score'] - base['score'] - rgb_baseline_known])
