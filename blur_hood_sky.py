#!/usr/bin/env python
"""
Sky + Hood Blur Preprocessing.

If PoseNet needs global structure (perspective lines, road, lane markings, horizon)
but we want to maximize compression in static/simple areas, what areas are safe?
1. Sky (top 15-20%) -> already proven safe
2. Car hood/dashboard (bottom 15-20%) -> very dark, static, contains no road structure
"""
...
def make_sky_hood_mask(sky_pct=0.15, hood_pct=0.15):
    """
    Returns an alpha mask where 1.0 = blur, 0.0 = keep original.
    """
    mask = np.zeros((H_CAM, W_CAM), dtype=np.float32)
    
    # Sky
    sky_end = int(H_CAM * sky_pct)
    if sky_end > 0:
        for y in range(sky_end):
            mask[y, :] = 1.0 - (y / sky_end) ** 0.5
            
    # Hood
    hood_start = H_CAM - int(H_CAM * hood_pct)
    if hood_start < H_CAM:
        for y in range(hood_start, H_CAM):
            mask[y, :] = ((y - hood_start) / (H_CAM - hood_start)) ** 0.5
            
    # Margins
    side_px = int(W_CAM * 0.03)
    for x in range(side_px):
        t = 1.0 - (x / side_px) ** 0.5
        mask[:, x] = np.maximum(mask[:, x], t)
        mask[:, W_CAM - 1 - x] = np.maximum(mask[:, W_CAM - 1 - x], t)
        
    return cv2.GaussianBlur(mask, (31, 31), 10)


def apply_mask_blur(frames, mask, sigma=5.0):
    alpha = mask[..., np.newaxis]
    ksize = int(sigma * 6) | 1
    return [(f * (1 - alpha) + cv2.GaussianBlur(f, (ksize, ksize), sigma) * alpha
...
if __name__ == '__main__':
    ...
    def test(name, sky_pct, hood_pct, sigma, crf='33'):
        mask = make_sky_hood_mask(sky_pct, hood_pct)
        # Save mask visualization
        cv2.imwrite(str(TMP / f'mask_{name}.png'), (mask * 255).astype(np.uint8))
        
        blurred = apply_mask_blur(orig_frames, mask, sigma=sigma)
        sz = encode_piped(blurred, out_mkv, crf=crf)
    ...
    print("\n=== Testing Sky + Hood Blur ===", flush=True)
    
    # Baseline (Sky only 15%)
    test("sky15_hood0_s5.0", 0.15, 0.0, 5.0)
    
    # Add hood blur
    test("sky15_hood10_s5.0", 0.15, 0.10, 5.0)
    test("sky15_hood15_s5.0", 0.15, 0.15, 5.0)
    test("sky15_hood20_s5.0", 0.15, 0.20, 5.0)
    test("sky15_hood25_s5.0", 0.15, 0.25, 5.0)
    
    # Try different sigmas for hood (it's dark, maybe stronger blur is okay)
    test("sky15_hood20_s8.0", 0.15, 0.20, 8.0)
    
    # Adjust CRF with hood blur (since we save more bits, maybe lower CRF is better?)
    test("sky15_hood20_s5.0_crf32", 0.15, 0.20, 5.0, crf='32')
    test("sky15_hood20_s5.0_crf34", 0.15, 0.20, 5.0, crf='34')
