#!/usr/bin/env python
"""Quick adversarial decode test for factored SegNet checkpoint."""
...
from svd_factored import factor_model, IDEAL_COLORS

device = torch.device('cuda')

print('Loading factored SegNet...')
model = SegNet().to(device)
model.load_state_dict(load_file(str(segnet_sd_path), device=str(device)))
factor_model(model, rank_ratio=0.2, verbose=False)
model.load_state_dict(torch.load('compressed_models/segnet_factored.pt',
                                  map_location=device, weights_only=True))
model.eval()
for p in model.parameters(): p.requires_grad_(False)

print('Loading teachers for eval...')
...
pose_t = torch.load('distill_data/pose_outputs.pt', weights_only=True).to(device)
del seg_logits
...
for st in [0, 200, 400]:
    ...
    tgt_s, tgt_p = ta[st:end], pose_t[st:end]
    B = tgt_s.shape[0]
    ...
    print(f'  seg={np.mean(results["ts"][-B:]):.6f} '
          f'pose={np.mean(results["tp"][-B:]):.6f}')
    del f0, f1, opt
    torch.cuda.empty_cache()

s = 100 * np.mean(results['ts'])
p = math.sqrt(10 * np.mean(results['tp']))
