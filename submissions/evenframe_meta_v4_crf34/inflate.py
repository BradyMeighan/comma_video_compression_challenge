...
"""
import sys, io, bz2, struct, av, cv2, torch, numpy as np
...
# ── REN v3: SegNet boundary correction at model resolution ──────────
...
def _infer_ren_channels(state_dict):
...
def get_ren_seg(archive_dir):
    global _REN_SEG
    if _REN_SEG is not None:
        return _REN_SEG
    for name in ['ren_seg_v3.int8.bz2', 'ren_seg_v3.pt']:
        p = Path(archive_dir) / name
        if p.exists():
            if str(p).endswith('.int8.bz2'):
                sd = _load_ren_int8_bz2(p)
            else:
                sd = torch.load(p, map_location=DEVICE, weights_only=True)
            ch = _infer_ren_channels(sd)
            _REN_SEG = RENv3(ch=ch).to(DEVICE).eval()
            _REN_SEG.load_state_dict(sd, strict=True)
            print(f"  Loaded REN-seg from {p} (ch={ch})")
            return _REN_SEG
    return None
...
def load_odd_frame_shift_chain(video_path: str):
    """
    Optional odd-frame geometric sidecars.

    Preferred packed chain:
      frame1_dxyr_chain_q.bin with payload <uint32 K><uint32 N><int8[K,N,3]>
    where int8 values are 0.1 model-px for dx/dy and 0.1 deg for theta.

    Fallback:
      frame1_dxyr_q*.bin, each a bz2-compressed int8 (N,3) in the same units.
    """
    base = Path(video_path).parent
    out = []

    p_chain = base / "frame1_dxyr_chain_q.bin"
    if p_chain.exists():
        try:
            blob = bz2.decompress(p_chain.read_bytes())
            if len(blob) >= 8:
                k, n = struct.unpack("<II", blob[:8])
                arr = np.frombuffer(blob[8:], dtype=np.int8).reshape(k, n, 3).astype(np.float32) / 10.0
                for i in range(k):
                    dx = arr[i, :, 0] * (camera_size[0] / MODEL_W)
                    dy = arr[i, :, 1] * (camera_size[1] / MODEL_H)
                    th = arr[i, :, 2]
                    out.append((dx, dy, th))
            return out
        except Exception:
    ...
    for p in sorted(base.glob("frame1_dxyr_q*.bin")):
        try:
            q = np.frombuffer(bz2.decompress(p.read_bytes()), dtype=np.int8).astype(np.float32).reshape(-1, 3) / 10.0
            dx = q[:, 0] * (camera_size[0] / MODEL_W)
            dy = q[:, 1] * (camera_size[1] / MODEL_H)
            th = q[:, 2]
            out.append((dx, dy, th))
...
def decode_and_resize(video_path: str, dst: str):
    ...
    odd_shift_chain = load_odd_frame_shift_chain(video_path)
    ...
    ren_seg = get_ren_seg(str(Path(video_path).parent))
    ...
    with open(dst, 'wb') as f:
        for frame in container.decode(stream):
            ...
            if H != target_h or W != target_w:
                ...
                # REN-seg: correct odd frames for SegNet via model-res residual
                if ren_seg is not None and (n % 2 == 1):
                    with torch.no_grad():
                        x_small = F.interpolate(x, size=(MODEL_H, MODEL_W),
                                                mode='bilinear', align_corners=False)
                        x_corrected = ren_seg(x_small)
                        # Compute residual at model res, upscale to full res, add
                        residual = x_corrected - x_small
                        residual_full = F.interpolate(residual, size=(target_h, target_w),
                                                      mode='bilinear', align_corners=False)
                        x = (x + residual_full).clamp(0, 255)
            ...
            elif odd_shift_chain or odd_ab_chain:
                pair_idx = n // 2
                arr = t.contiguous().numpy()
                changed = False
                if odd_shift_chain:
                    center = (target_w * 0.5, target_h * 0.5)
                    for dx_arr, dy_arr, th_arr in odd_shift_chain:
                        if pair_idx < dx_arr.shape[0]:
                            dx = float(dx_arr[pair_idx])
                            dy = float(dy_arr[pair_idx])
                            th = float(th_arr[pair_idx])
                            if abs(dx) > 1e-6 or abs(dy) > 1e-6 or abs(th) > 1e-6:
                                M = cv2.getRotationMatrix2D(center, th, 1.0)
                                M[0, 2] += dx
                                M[1, 2] += dy
