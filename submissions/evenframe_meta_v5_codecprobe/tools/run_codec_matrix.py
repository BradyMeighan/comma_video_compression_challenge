#!/usr/bin/env python
"""
Run codec-centric ablations from codec_optimization.md claims.

This is codec-only screening (no sidecars, no REN) to identify whether
there is real baseline headroom before expensive retraining/re-optimization.
"""
...
CASES = [
    # Reference around known strong ffmpeg/libsvt setup.
    CodecCase(
        name="ref_8bit_crf33_fg22_k180_lanczos_unsharp045",
        scale_w=522,
        scale_h=392,
        pix_fmt="yuv420p",
        preset=0,
        crf=33,
        svt_params="film-grain=22:keyint=180:scd=0",
        decode_filter="scale=1164:874:flags=lanczos,unsharp=5:5:0.45:5:5:0.0",
    ),
    # Claimed P0-like recipe.
    CodecCase(
        name="p0_like_10bit_crf34_p1_fg30_dn0_keyinf_bicubic_us2",
        scale_w=582,
        scale_h=436,
        pix_fmt="yuv420p10le",
        preset=1,
    ...
    # 8-bit ablation of same.
    CodecCase(
        name="p0_like_8bit_crf34_p1_fg30_dn0_keyinf_bicubic_us2",
        scale_w=582,
        scale_h=436,
        pix_fmt="yuv420p",
        preset=1,
    ...
    # Keyint ablation.
    CodecCase(
        name="p0_like_10bit_crf34_p1_fg30_dn0_k180_bicubic_us2",
        scale_w=582,
        scale_h=436,
        pix_fmt="yuv420p10le",
        preset=1,
    ...
    # Film grain denoise ablation.
    CodecCase(
        name="p0_like_10bit_crf34_p1_fg30_defaultdn_keyinf_bicubic_us2",
        scale_w=582,
        scale_h=436,
        pix_fmt="yuv420p10le",
        preset=1,
        crf=34,
        svt_params="film-grain=30:keyint=-1:scd=0",
        decode_filter="scale=1164:874:flags=bicubic,unsharp=5:5:2.0:5:5:0.0",
    ),
    # Film grain off ablation.
    CodecCase(
        name="p0_like_10bit_crf34_p1_fg0_keyinf_bicubic_us2",
        scale_w=582,
        scale_h=436,
        pix_fmt="yuv420p10le",
        preset=1,
        crf=34,
        svt_params="film-grain=0:keyint=-1:scd=0",
        decode_filter="scale=1164:874:flags=bicubic,unsharp=5:5:2.0:5:5:0.0",
    ),
    # Decode filter ablation.
    CodecCase(
        name="p0_like_10bit_crf34_p1_fg30_dn0_keyinf_lanczos_us045",
        scale_w=582,
        scale_h=436,
        pix_fmt="yuv420p10le",
        preset=1,
...
def main() -> None:
    ...
    for idx, case in enumerate(CASES, start=1):
        ...
        enc_cmd = [
            ...
            str(VIDEO),
            "-vf",
            f"scale={case.scale_w}:{case.scale_h}:flags=lanczos",
            "-pix_fmt",
            case.pix_fmt,
            "-c:v",
            "libsvtav1",
        ...
        # Package archive.zip (only compressed video)
        with zipfile.ZipFile(zpath, "w", compression=zipfile.ZIP_STORED) as zf:
            zf.write(mkv, "0.mkv")
        archive_bytes = zpath.stat().st_size

        # Decode/inflate path with explicit filter chain.
        dec_cmd = [
            "ffmpeg",
            "-y",
            "-hide_banner",
