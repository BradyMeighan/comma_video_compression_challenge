...
def main():
    ...
    # Warmup torch.compile by running a dummy forward+backward at batch_size
    # This pays the JIT cost upfront (~60-90s) instead of during batch 1 (~250s)
    if _compiled and device.type == 'cuda':
        ...
        for _ in range(3):
            with torch.amp.autocast('cuda'):
                ...
                p_loss = pn_out_w.mean()
                total_w = s_loss + p_loss
            total_w.backward()
        del dummy_seg, dummy_f1, dummy_f0, s_out, s_loss, both_w, pn_in_w, pn_out_w, p_loss, total_w
        torch.cuda.empty_cache()
