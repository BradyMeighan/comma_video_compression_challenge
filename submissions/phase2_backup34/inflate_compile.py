...
def optimize_batch(
    ...
    for it in range(num_iters):
        ...
        # Set LR directly (no scheduler object)
        ...
        # Only sync to CPU every CHECK_INTERVAL iters (reduces GPU stalls)
...
def main():
    ...
    # Regional torch.compile — compile only the model backbones, not the loop
    ...
    print(f"Compile setup in {time.time() - t_compile:.1f}s (actual compilation happens on first call)")
