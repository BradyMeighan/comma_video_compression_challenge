...
def optimize_batch(
    ...
    f0_init = init_odd.detach().mean(dim=(-2, -1), keepdim=True).expand_as(init_odd).contiguous().clone()
    ...
    # Phase 1: AdamW on both frames, SegNet-only (30% of iters)
    ...
    # Phase 2: L-BFGS on both frames (combined SegNet + PoseNet loss)
    lbfgs_iters = num_iters - early_cutoff
    lbfgs = torch.optim.LBFGS(
        [frame_0, frame_1], lr=1.0, max_iter=1,
    ...
    for it in range(lbfgs_iters):
        progress = (early_cutoff + it) / max(num_iters - 1, 1)
        a_eff = alpha if progress < 0.9 else alpha * 0.2
        b_eff = beta * 1.5 if progress < 0.9 else beta * 8.0
        ...
        def closure():
            ...
            total.backward()
            # L-BFGS uses .view(-1) on grads which fails on non-contiguous tensors
            for p in [frame_0, frame_1]:
        ...
        if it % 3 == 0:
            with torch.no_grad():
                seg_logits = segnet(frame_1)
                seg_loss_val = margin_loss(seg_logits, target_seg, seg_margin).item()
                both = torch.stack([frame_0, frame_1], dim=1)
                pn_in = posenet_preprocess_diff(both)
                pn_out = posenet(pn_in)['pose'][:, :6]
                pose_loss_val = F.smooth_l1_loss(pn_out, target_pose).item()

        with torch.no_grad():
            if it >= lbfgs_iters - AVG_LAST_K:
                f0_accum += frame_0.data
                f1_accum += frame_1.data
                avg_count += 1

    actual_iters = early_cutoff + lbfgs_iters
    ...
    del lbfgs, frame_0, frame_1, f0_accum, f1_accum, init_odd
