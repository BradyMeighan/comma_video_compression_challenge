"""Final ideas: last-K averaging, different color init, noise injection."""
...
ideal_colors_default = torch.from_numpy(ideal_colors_np.copy()).float().to(device)
...
def custom_optimize(
    ...
    colors = color_init if color_init is not None else ideal_colors_default

    init_odd = colors[target_seg].permute(0, 3, 1, 2).clone()
...
print('\n=== NOISE INJECTION ON INIT ===')
for sigma in [1.0, 3.0, 5.0, 10.0, 20.0]:
    evaluate(f'noise_sigma={sigma}', noise_sigma=sigma)

print('\n=== ALTERNATIVE COLOR INIT ===')
# Maximally separated colors (equidistant in RGB space)
alt_colors1 = torch.tensor([
    [0.0, 0.0, 0.0],
    [255.0, 0.0, 0.0],
    [0.0, 0.0, 255.0],
    [255.0, 255.0, 255.0],
...
# All same (gray)
alt_colors3 = torch.full((5, 3), 128.0, dtype=torch.float32, device=device)

evaluate('max-separated colors', color_init=alt_colors1)
evaluate('grayscale ramp', color_init=alt_colors2)
evaluate('all gray', color_init=alt_colors3)
